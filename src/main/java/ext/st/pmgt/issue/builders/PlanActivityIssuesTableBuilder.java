...
/**
 * @author: Mr.Chen
 * @create: 2021-01-21 14:46
 **/
public class PlanActivityIssuesTableBuilder extends AbstractComponentBuilder {
    PIProject project=null;
    @Override
    public Object buildComponentData(ComponentParams params) throws PIException {
        Object contextObject = params.getNfCommandBean().getSourceObject();
        PIPlanActivity act = null;
        if(contextObject instanceof PIPlanActivity){
            act=(PIPlanActivity)contextObject;
        }else if (contextObject instanceof PIResourceAssignment) {
            PIResourceAssignment assignment = (PIResourceAssignment)contextObject;
            act = (PIPlanActivity) assignment.getPlannable();
        }
        project=act.getProject();
        return STProjectIssueHelper.service.getProjectIssues(act);
    }
    ...
    public ComponentConfig buildComponentConfig(Object componentData, ComponentParams params) throws PIException {
        ...
        if (isManager(project)) {
            tableConfig.setRightMenuName("projectRelatedIssuesMenusCopyByManager", params);
        } else {
